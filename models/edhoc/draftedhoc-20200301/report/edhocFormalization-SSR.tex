...
running and commit action facts (\mT{CommitI} and \mT{CommitR} respectively).
%
\\
...
lemma authGIYImplicitAuthGuaranteeForI:
    ...
    "
\end{lstlisting}
As opposed to lemma \mT{authInjAgreeGuaranteeForI}, we here prove that the two
parties implicitly authenticate on the keys \mT{impSk}.
%
In this lemma we show that if any two parties (\mT{u} and \mT{v2} here) both
has completed a run of the protocol, and \mT{u} believes she is talking to
