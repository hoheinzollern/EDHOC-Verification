...
rule R2_STAT_SIG:
 ...
 76 exp_sk = <gx^~yy>
\end{lstlisting}
\knote{Please ignore listing above. It is WIP.}
\knote{@Vaishnavi: I promised to commit what I have at 18:00 CET. Section 3.3
    is done except for weaving in the listing above. I have not really started
    on Section 3.4 yet.
}
\vnote{Did you mean section 3.4 and 3.5 respectively? :) Yes, that's fine. You can submit a version tonight, but I'll go through section 3.5 my tomorrow morning, just to be sure, and correct grammatical errors and run-on sentences (if any).}
...
We mark some steps of the protocol with \mT{Running} action facts.
%
...
%
For example, the fact \mT{ExpRunningR(~tid, \$V, exp_sk)}
...
%
Specifically, \mT{imp_sk} excludes key material which it is even theoretically
