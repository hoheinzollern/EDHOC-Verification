...
term/.style={rounded corners},
  ...
  cond/.style={base, draw=black, fill=none, diamond},
  txt/.style={base, draw=none, fill=none}
  }
% -------------------------------------------------
% Start by placing the nodes
%\node [terminput] (u1) {Seed};
...
\draw [->, norm] (h8) -- (k2b);
\draw [->, norm] (p2) -- (h2); 
\draw [->, norm] (c1.east) -- ++(2,0) -- (h3.north) node[midway,above left] {Y};
%\draw [->, norm] (h3.south) -- ++(0,-1) -- ++(-3,0);
\draw [->, norm] (h3.west) -- (p3.east);
\draw [->, norm] (p3) -- (h4); 
\draw [->, norm] (c2.east) -- ++(2.05,0) -- (h5.north) node[midway,above left] {Y};
...
\node [dhk, right=2cm of h1] (p0) {$\mGxy$};
\node [terminput, text width=2em, below = 0.3cm of h1] (u1) {Seed};
\draw [->] (h1.west) -- (p2.east);
\draw [->] (u1.north) -- (h1.south);
\draw [->] (p0.west) -- (h1.east);


\node [dhk, right = 2cm of h3] (u2) {$\mGrx$};
\draw [->, norm] (u2.west) -- (h3.east);

\node [dhk, right = 2cm of h5] (u3) {$\mGiy$};
\draw [->, norm] (u3.west) -- (h5.east);


\node [term, above = 1.3cm of h4] (u5) {\mTHtwo};
...
\node [term, below=0.42cm of p4] (u7) {\mTHfour};
%\draw [->, dotted, shorten >=1mm] (u7) -- (h9.south);
\draw [-> , dotted ] (u7.west) -- ([yshift=-0.4em] h9.east);

\matrix [draw, ultra thick, double, below=3em of t1.east] {
