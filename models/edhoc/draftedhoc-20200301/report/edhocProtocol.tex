...
%\vnote{I realize that we use subsubsection a lot both in this subsection and the next one, which eats up enormous amounts of vertical space before the heading. Given that we do not refer to these sections in the text, it might be better to make them running heads in bold, rather than subsubsection.}
...
%
\knote{We need the last sentence above. That is what gives the implicit
    authenticity guarantee of the session key.}
\vnote{Ok.}
...
\vnote{But they are separate elements, which is enough to convince anyone that the guarantees cannot be borrowed directly. Why they are kept separate is immaterial, and actually distracting to someone like me who does not offhand know what ``separation of concerns principle'' means.}
while \mEdhoc{} uses the ephemeral DH half-key itself as challenge.
%
\\
%------------------------------------------------------------------------- sub
...
\knote{I'm consistently removing quotation marks as you have noticed. To me
    quotation marks signal unclarity. For example here if we write
    ``patterns'', I would wonder if they are not really patterns, only similar
    to patterns.
}
\vnote{The quotation marks are there to signify that this is a notation used in Noise, and not some generally known notation. It's fine if you think they shouldn't be used.}
%
...
\begin{itemize}
    ...
    %\item Compromise of the \mHkdf{} input parameters (\mGxy{} shared
            ...
            %\knote{I don't know which property the above tries to capture.}
