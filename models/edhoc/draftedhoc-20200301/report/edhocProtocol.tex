...
\kedit{Agents}{Parties} participating in the \mEdhoc{} protocol can authenticate
...
Four combinations of the \mSig{} and \mStat{} authentication methods are
possible\kedit{}{ for the two parties},
while the \mPsk{} authentication method can only be run by \kedit{an agent with another
agent}{a party when the other party is} also authenticating with a \mPsk.
We refer to these combinations also as methods, denoted \mSigSig, \mSigStat,
\mStatStat, \mStatSig{} and \mPskPsk.
\kedit{}{We refer to a method where at least one party uses \mSig{} as a
    \mSig{} based method and similarly for \mStat{} and \mPsk.
}
...
\knote{I removed paragraph break here. The above sentence could start off the
next paragraph.}
...
The \mSig{} based methods of \mEdhoc{} \kedit{are built on}{are derived from} \mSigmaI, a variant of the
\mSigma{} protocol which provides identity protection for the initiator
\kedit{, and implements the \mSigmaI variant as Sign-then-Mac.}{. However, \mEdhoc{} deviates from \mSigmaI{} in several aspects,
e.g., \mEdhoc{} uses Mac-then-Sign like \mTls{} instead of Sign-then-Mac.}
\kedit{The method involving static DH keys}{The \mStat based methods use
challenge-response signatures using static Diffie-Hellman keys and} proceed
along the lines of \mOptls\kedit{, with the agent creating a MAC and encrypting it using an algorithm for authenticated encryption with associated data (\mAead)}{. The challenge-response signatures use
ephemeral Diffie-Hellman keys as challenges and an \mAead{} MAC to protect
the data to be signed~\cite{aead,rfc5116,DBLP:conf/eurosp/KrawczykW16}.}
...
\subsection{\kedit{Comparison with}{Relations to} \mSigma, \mOptls{} and \mNoise{}}
\subsubsection{\mSigma{}}
As noted above, the \mSigSig{} based method of \mEdhoc{} is a very close
derivative of \mSigmaI{} and was carefully analyzed by
Bruni~et.~al.~\cite{DBLP:conf/secsr/BruniJPS18}.
So was the \mPskPsk{} method of \mEdhoc{}. Therefore, we will not focus on those
in the reminder of this paper. We do however model them to the same extent and
verify their properties with the same care as the other methods.
\subsubsection{\mOptls{}}
\knote{BTW, I silently added \{\} to all macros I found that needed it for
trailing space}
\mOptls~\cite{cryptoeprint:2015:978} is a key-exchange protocol developed \kedit{
upon which to base the design of the}{with the intention to serve as a basis for
the} handshake protocol in \mTls\ 1.3. \kedit{}{\mOptls{} is a one round-trip
protocol providing server to client authentication, but not vice versa.}
\kedit{In its most basic form, \mOptls{} is
composed of two messages between a client and a server, where the} The server has a
secret static key $s$ and a \kedit{}{public static} DH key $g^{s}$. The client
first sends to the server a \kedit{}{challenge} message composed of a fresh nonce and some
negotiated parameters, along with an \kedit{ECDH}{ephemeral DH} key $g^{x}$. The
server responds with a nonce of its own, some other negotiated parameters, an
\kedit{ECDH}{ephemeral DH} key $g^{y}$, and a MAC of all these values computed
using a key derived from $g^{xs}$. At the end of the protocol, a session key is
derived \kedit{jointly}{by each party} from $g^{xs}$ and $g^{xy}$. The former
ensures secrecy for as long as $s$ \kedit{is secure}{remains secret}, and the latter provides perfect forward secrecy in case $s$ is compromised.
\kedit{}{The core idea is that, when receiving the response message, the client
    deduces that only the intended server could have
    computed $g^{xs}$ with the knowledge of $s$,  and from $g^{xs}$ and $g^{xy}$
    derived the MAC key.  Under this assumption, the client further deduces that
    anything covered by the MAC is originating from the intended server.

    This construction is sometimes referred to as a challenge-response signature.
    It is very efficient in terms of the number of transmitted bits.
}
\kedit{It can be seen that in}{In} \mEdhoc, \kedit{an \mStat role is very similar to being either the client or the server in an \mOptls execution.}{a party authenticating with
    with the \mStat{} method essentially acts as an \mOptls{} client and the other
party as an \mOptls{} server. There are differences, e.g., that \mEdhoc{} reuses
the $g^x$ as a nonce, while \mOptls{} makes an explicit point about keeping
these as separate elements.}  The \mStatStat{} method can be thought of as interleaving two
\mOptls{} sessions\kedit{.}{, where the second \mEdhoc{} message carries both the response
...
of security protocols where all parties \kedit{communicate}{authenticate} using
static DH keys\kedit{}{, and} whose structure also meets certain requirements of encryption etc. 
...
of the \mNoise{} framework to \kedit{see if there is any}{determine the} correspondence with the
\mStatStat{} method.
...
encryption/decryption keys. We give a quick introduction to \mCose{} and
\mAead{} for the sake of completeness, and we then flesh out the workings of the
KDF\kedit{in full detail}, before moving on to discuss the \mEdhoc{}\kedit{}{ overall
structure and the details of the} methods.
...
\subsubsection{\mAead{}}
Authenticated encryption with associated data, or \mAead{}, \kedit{is an authenticated
    encryption operation}{is, as the name suggests an operation that provides
both data authentication and encryption}~\cite{aead}. The \kedit{}{encryption} algorithm takes in a
...
The associated data \vedit{is used to protect information that needs to be authenticated, but does not need to be kept confidential. It might be desirable to authenticate this}{field contains} information \vedit{}{which it might be desirable to authenticate, but} \vedit{, although it}{} must be left unencrypted to allow the system to function properly.
\knote{Is the two previous sentences not saying the same thing?}
\vnote{Sure, I've removed one.}
Authentication \kedit{}{of associated data} is provided without \kedit{copying the data into the
plaintext}{including it in the ciphertext.}.

\kedit{}{The \mAead{} decryption operation returns the decrypted ciphertext if
    the MAC verifies. If the MAC fails to verify, the operation returns
nothing.}
...
\vedit{This is a great point, and one I had thought about, but forgot to fix after I had added the figure. I'll do that.}
