...
material.
%
% KARL: this comes in section 2 anyway:
%For the other \mStat{}-based methods, the party using the
...
%There are differences between \mOptls{} and the \mStat{}-based methods.
%
...
\mEdhoc{} does not use all functions of \mNoise{} exactly as prescribed, which
...
{\color{red}I have a note here, but for some reason notes no longer appear in
the PDF.}
\knote{I really like the new text in the diamond shapes. Much clearer. The
    numbering is however still used under the \mMactwo{} and \mMacthree{}
    symbols. How about changing the text under \mMactwo{} to if R uses SIG and
corresponding for \mMacthree{}?
}
...
\begin{itemize}
    ...
    \item \textbf{protection against replay attacks} by the attacker (we {\color{red} VAISHNAVI: Incomplete sentence?}
        {\color{red}KARL: It could be a left-over from the claim I did not
            understand. The only claim of protection against reply in the draft
            is on page 33 where EDHOC is compared to SIGMA. However, the given
            argument is vague and the claim seems untrue: SIGMA allows for
            including signed "additional information" and this is exactly what
            EDHOC uses to protect algorithm choices. The spec also talks about
            protecting previous messages, which may be referring to the
            transcript hash, but I don't see what that has to do with replay
            attacks. I propose we delete the item "protection against replay
            attacks" from here or we write a sentence or two about
        that it is unclear.}
