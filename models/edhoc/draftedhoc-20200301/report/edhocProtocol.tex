...
\knote{This section was mainly rephrasing text from the \mSpec{} in prose
    form. It is not easy to see how we should connect back to that clearly from
    the analysis and discussion sections.
    I have made a first itemization of the text, but propose that we go even
    further and just bullet list the claimed properties, and for the ones that
    are unclear we put in an interpretation and highlight that it is unclear.
    Then we can tick-off in the conclusion what we have checked, what not and
    what we have checked in addition.
    Views on that approach?
}

In this section, we list the claims made by the authors
...
\begin{itemize}
    ...
    \item \textit{For the \mPskPsk{} method, compromising \mPsk{} lets the
        ...
        \knote{The above is a convoluted, and possibly incorrect, phrasing of
    ...
    \knote{The first sentence is unclear. I don't know which property it refers
    to. The second is clear though: independence between session keys.}
\item \textit{
        ...
        \knote{The above seems to be PFS again, which was mentioned already in
        he the first bullet. I suggest we keep only one.}
\end{itemize}
...
\knote{Need to revise the above paragraph when decided what to do about this
section.}
