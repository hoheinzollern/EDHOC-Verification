...
\kedit{}{The core idea is that, when receiving the response message, the client
    ...
    This structure is sometimes referred to as a challenge-response signature.
    It is very efficient in terms of the number of transmitted bits.
}
It can be seen that in \mEdhoc, \kedit{an \mStat role is very similar to being either the client or the server in an \mOptls execution.}{a party authenticating with
...
\knote{There is something odd with the sentence above. "Same key" and then "one
depending on ... second depending on.."}
