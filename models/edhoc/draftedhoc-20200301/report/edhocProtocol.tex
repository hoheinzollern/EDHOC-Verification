...
%
%To reach this goal, 
Here, we provide a quick introduction to the \mEdhoc{} protocol. For more details, we refer the reader to~\cite{selander-lake-edhoc-01}. To establish an authenticated security context for \mOscore,
...
ephemeral DH half-key \mGx{}, along with some parameters to establish an \mOscore{} security context (supported authentication method \mMethod, supported cipher suites \mSuites, and connection identifier \mCi).\footnote{While we present these parameters here while describing \mEdhoc, we will ignore them in our formal modelling, in Section~\ref{sec:formalization}.} \mT{R} may reject the choice of the authentication method or cipher suites, leading to a negotiation across multiple \mEdhoc{} sessions.
...
DH half-key \mGy{} and its connection identifier \mCr, and authenticates itself to \mT{I} by
...
an endpoint may use for authentication: certificates, private keys
...
%
We overload notation to refer to these \emph{combinations} of authentication methods as
...
authentication method used by the initiator of the protocol run and the second the one used by the responder.)
%
...
The key schedule is rooted in the ephemeral DH key \mGxy{}, the combination
...
derived.
%
For \mPskPsk{}, the pre-shared key is used as seed for generating \mPRKtwo.
%
...
long-term key of the endpoint (or endpoints, if both) using \mStat{}-based
authentication.
\begin{figure}[h]
\centering
\scalebox{.785}{
...
%
However, a detailed description of the \mSigSig{} and \mPskPsk{} methods can be found in the
technical report at~\cite{edhocTamarinRepo}. 
\vnote{Ensure that this is put in.}
%
...
The message sequence chart for this method can be found in Appendix~\ref{sec:mscstatsig}.
%
...
\label{sec:claimedProperties}
 ...
 \cite{selander-lake-edhoc-01} claims that \mEdhoc{} satisfies many security properties, but some of these are imprecisely expressed.
%
