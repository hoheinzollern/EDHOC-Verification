...
\title{Formal Analysis of EDHOC Key Establishment for Constrained IoT Devices
    % This was suggested by *another* Springer lncs guideline. Correct according
    % to the lncs guideline linked by SSR is to use a section with run-in heding
    %\thanks{This work was partially supported by the Wallenberg AI, Autonomous
        %Systems and Software Program (WASP) funded by the Knut and Alice
        %Wallenberg Foundation.
        %%
        %We are grateful to G\"oran Selander, John Mattsson and
        %Francesca Palombini for clarifications regarding the specification.
    %}
}
%
...
\knote{Vaishnavi, your text editor is again making every paragraph into a single
    text-file line. Please change the setting if possible.
}
\vnote{There is no setting that inserts comment marks after every sentence! I can keep a setting which doesn't convert whatever you send, but I find it really tough to write like that myself. In any case, I'm not going to write anymore, I assume, so it should be okay?}
...
\vnote{I would like to move the next paragraph to Section 3, where we define Authentication.
It fits in better with where we do talk about implicit/explicit authentication,
rather than as a commentary on the work that has been done for Sigma etc.
(Why should implicit-vs-explicit authentication matter for whether one can
directly adopt those proofs of ``security'' or not?)}
\knote{You could be right. What I tried to capture is that we are not the first
    ones to use implicit authentication in the symbolic model, even though doing
    so is unusual compared to using local-view injective-agreement style
    authentication. So it felt right to point out related work in this area.
}
\vnote{It's related only in the sense of the properties we verify, and I really feel like it only belongs as part of that discussion. There's going to be a lot of stuff that we cite as precedence, all of it doesn't need to show up in related work.}
...
%
\knote{substitute \cite{personalCommunication} for
"personal correspondance with the authors".}
...
\knote{Why are runhead used instead of subsubsection? I did not find guidelines
    regarding that. The result seems to be the same.
}
\vnote{The vertical space used by runhead seems to be lesser. There is no guideline that says you can't do this. }
...
%attacks and the responder's against passive attacks
%%
%However, it also states that the roles should be assigned to the \mCoap{}
%peers to protect the most sensitive identity the most
%(see Section~8~\cite{selander-lake-edhoc-01}).
%%
%Roles could hence be swapped, so identity protection for the most sensitive
%identity can be maintained.
%\vnote{I feel like we can remove this discussion about identity protection. It can be handled, so the only problem is the message size.}
%
...
\vnote{This is if the ``problem'' identified above is not a problem at all! Can remove.}
\knote{Not sure I understand. We are listing four alternative solutions, not
    problems. One alternative is to make an informed decision to accept
    the situation even though it is problematic/undesirable (which is very
    different from ignoring it or not being aware of it).
    So, I don't think we should remove this alternative.
}
\vnote{It says maybe the security problem being talked about isn't really a problem, because we choose to see things differently. Need to reword  to add this before we give potential solutions to the problem of \mGiy.}
%
