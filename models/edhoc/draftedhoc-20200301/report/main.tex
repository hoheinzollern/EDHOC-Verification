...
%heterogeneous security properties.
%
...
% KARL: ESORICS 2021 call does not discuss keywords. Some papers from previous
% years have them and some don't.
\keywords{Formal verification \and Security \and IETF standardization \and
Key establishment protocols \and Tamarin}

\knote{
    \texttt{https://esorics2021.athene-center.de/call-for-papers.php}\\
     - All submissions must be received by 11:59 p.m. Berlin, Germany\\
            Time on the day of the corresponding deadline.\\
     - \textbf{Next Deadline: Title and abstract: January 05, 2021} or May 05, 2021\\
     - Full paper submission:  January 12, 2021 or   May 12, 2021\\
     - Submitted papers should be at most 16 pages (using 10-point\\
            font), excluding the bibliography and well-marked appendices, and
            at most 20 pages total.\\
\textbf{Is prior arXiv publication acceptable for ESORICS? unclear}
}
\vnote{Not sure. Look it up, and if we're still unsure we can email the PC chairs.}
\knote{The call-for-papers says: \emph{Submitted papers must not substantially
    ...
    This is not entirely clear to me. It seems to be OK, but your opinion is
appreciated.}
...
%
%Howeverr, \mCoap{} does not include security protection on its own, and in some cases,
%\mDandTls{} messages are too large to fit into the radio frames, or unsuitable to handle proxy operations with change of underlying transport protocol.
%
...
variant of challenge-response signatures.%, in the style of the \mNoise{} framework~\cite{perrin2016noise}.
%
...
in May 2018.
%
In the current version of the \mSpec{}, the \mPskPsk{} method has been removed.
%
In our analysis in~\cite{edhocTamarinRepo}, we also analyze the last known
variant of the \mPskPsk{} method, from~{\textbf{*** Put in the right link to the
spec version here ***}}, since it might be added again, e.g., as a basis for
fast re-establishment.
%
We present a quick overview of the same in the appendix, to easy understanding
of Section~\ref{sec:formalization}.
%
\vnote{Karl, please check that this footnote makes sense. Do we have any better justification?}
%by
%integrating the cryptographic core of \mOptls{}.
%
\knote{Mads proposes starting the following paragraph with "since there has been a lot of change..." and
then explain what. Also highlight difference to Bruni.}
More importantly, mixed variants where one party uses a challenge-response
signature and the other a regular signature have also been added.
%
Consequently, the protocol has undergone a lot of change, and formal analysis is
...
2020-11-02~\cite{selander-lake-edhoc-01}, with the \mPskPsk{} method based on the version as on \textbf{*** Fix this reference ***}.
%
...
%
%This analysis has also resulted in some improvements to the \mSpec.%, 
...
different methods relate to (non)-repudiation~\cite{selander-lake-edhoc-01}.\\
...
\knote{More standards impact: -02 draft: 4th message to get key confirmation
    included as an option. Use-case section included in standard.
}
\knote{We also modelled PFS and a form of integrity protection for \mT{AD_3}. 
...
\begin{table}[h!]
    \begin{center}
        \caption{Summary of verified properties. Subsumed properties are shown
...
\vnote{Cite the right version of the draft in the preceding paragraph. Something about the properties here?}

% \section{Abstract model of \mEdhoc{} communications}
% \begin{figure}
% \centering
...
%     \msg{1em}{ini}{res}{\mMethod, \mSuites, \mGx, \mCi, \mADone};
% \msg{3em}{res}{ini}{\mCi, \mGy, \mCr, [\mIdcredr, \mAuthr, \mADtwo]};
%     \msg{5em}{ini}{res}{\mCr, [\mIdcredi, \mAuthi, \mADthree]};
