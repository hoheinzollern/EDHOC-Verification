...
\end{abstract}
%
\knote{The last sentence says "also". That only makes sense in reference to the
    previous, now deleted sentence.  I propose to remove the "also", or add a
    sentence before it explaining that we have shown agreement properties and
PFS}
...
% VAISHNAVI: I don't think there is any need to add keywords
...
%
%Howeverr, \mCoap{} does not include security protection on its own, and in some
% cases, \mDandTls{} messages are too large to fit into the radio frames, or
...
%\knote{They removed XOR with HKDF-countermode as a key stream generator and
%    substitute it for a real encryption function. We should probably update our
%model with this.}
%
...
In this work, we have analyzed the version of the \mEdhoc{} protocol as presented in the \mSpec{} document dated 2020-11-02~\cite{our-analysis-selander-lake-edhoc-01} (along with the \mPskPsk{} mode as presented in~\textbf{*** Enter proper reference here ***}). 
...
As mentioned earlier, a previous version of \mEdhoc{} allowed for a \mPsk{} mode, based on pre-shared keys. 
While any combination of the \mSig{} and \mStat{} authentication methods is possible, when \mPsk{} is used, both endpoints must use \mPsk{}, giving rise to the \mPskPsk{} method. 
The key schedule is followed exactly as usual here as well, except that the seed for generating \mPRKtwo is no longer empty, but the pre-shared key.
All three intermediate keys are the same. 
The message sequence chart is presented in Figure~\ref{fig:edhocpsk}, and the following properties were verified to hold for this method: \textbf{*** List properties here ***}
