...
shares with many other standards.
%
...
the specification makes a number of detailed claims of properties
...
in the paper analyzing \mSigma~\cite{DBLP:conf/crypto/CanettiK02} can be copied
into the specification.
%
(Note: likely the intention is the same for the other reused academic protocols,
but since the specification is still work in progress, it is not yet written
down).
%
...
%
However, \mSigma and the other cryptographic cores used by \mEdhoc, are
typically designed for a precise setting with many underlying assumptions.
%
Without spelling those out in the specification, and without showing how
...
Another aspect showing that usefulness of considering \mEdhoc light-wight
use-cases is that, until we pointed it out, the specification did not contain
...
group requirements draft\knote{citation}.
%
It would, however, be useful to extend the security considerations section in
the specification with example use-cases and showing the consequences of
...
%-------------------------------------------------------------------------- sub
\subsection{Unclear identity management}
\label{sec:usableSecurity}
\mPoint{Protocol is easy to use incorrectly/insecurely; protocol is unclear
about what an identity that is authenticated is}
