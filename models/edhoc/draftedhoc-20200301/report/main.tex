...
\todo[inline, color=red, size=\footnotesize]{\textbf{\color{white}Please
disable your text-editor's line-concatenation feature. Thanks!}}
...
\mTamarin{} tool.
%
We analyze \mEdhoc{} with respect to two authentication properties and
Perfect Forward Secrecy of the session key material.
%
In the course of the analysis, we also found some potential improvements to the
design and specification of \mEdhoc, which we have communicated to
the designers.
%
\end{abstract}
%
\vnote{Yes, I do think we need to add a sentence about what properties we've proven. I think we can remove the keywords though. Do you think it might help to change the title to ``Formal Verification'' instead of ``Formal Analysis''? I feel like the latter is more standard terminology in this community.}
\knote{To me formal \emph{verification} indicates that someone else has already defined
    what should be verified and you just verify that that is true, which I
    think is selling ourselves short in this case. A large part of our work
    has been to identify what makes sense to verify.
    But, if you think it sounds strange to someone from the FM community,
    please change it.}
\knote{I agree we don't need the keywords, but I'd prefer to keep them if there
    is space left. Doing so increases the chance that our paper turns up in
    web-searches related to formal methods and standardization.}
\vnote{The link we have in [15] is the generic link to the EDHOC spec drafts, and it says accessed in March (we say accessed in November in the text instead). I feel like it might be better to actually link to draft-ietf-lake-edhoc-02 instead, which certainly came out in November, and which is what we mention in the text as well. It also makes sense to add a reference to the older version (01?) so we can cite that while talking about PSK. Let me know what you think.}
\knote{That citation  is intentionally like that. It points to the repo and the access
time gives an exact version of the document we analyzed. It is not exactly the
same as -02 and it is not exactly the same as -01.  The link and access time
identify exactly the PSK version we analyzed.
In the paper we mention three versions of the EDHOC draft (and I tried to make it
super-clear in the bib-file which is which by naming the bibtext-identifier
appropriately).
One of them is the one we analyzed~\cite{our-analysis-selander-lake-edhoc-01},
another is the latest version of the draft~\cite{latest-ietf-lake-edhoc-03},
and the third one is the one Bruni et al
analyzed~\cite{bruni-analysis-selander-ace-cose-ecdhe-08}.
We need all three to refer to
them correctly when discussing who has done what and what has not yet been done.}
...
As IoT devices become more prevalent and get involved in evermore sensitive
...
%
It is therefore important to increase assurance that \mEdhoc{} satisfy security
...
\mSpec{} we analyze~\cite{our-analysis-selander-lake-edhoc-01}, and later
versions, lack a clear description of the intended security model and
overall security goals.
%
The standardization process is ongoing, releasing new versions of the draft
\mSpec{}.
%
...
%2020-03-01~\cite{our-analysis-selander-lake-edhoc-01}, with the \mPskPsk{} method based on the version as on \textbf{*** Fix this reference ***}.
%
...
\vnote{I think it would be good to add a note about why the modelling in Tamarin is difficult here. Right now it feels like a very brusque paragraph, and it could use some detail, and it makes perfect sense to put it here.}
%
...
\begin{table}[h!]
    \begin{center}
        ...
        \begin{tabular}{|l|c|c|c|c|c|}
                ...
                %\hline
                %Mutual entity authentication & \cm & \cm & \cm & \cm & \cm\\
                %UKS & \cm & \cm & \cm & \cm & \cm\\
                %KCI & \cm & \cm & \cm & \cm & \cm\\
                \hline
        \end{tabular}
...
% KARL: I don't think we need this section now. Please see the introduction.
%\subsection{Newer version of the \mSpec}
%In this work, we have analyzed the version of the \mEdhoc{} protocol as
%presented in the \mSpec{} document dated 2020-03-01~\cite{our-analysis-selander-lake-edhoc-01} (along with the \mPskPsk{} mode as presented in~\textbf{*** Enter proper reference here ***}). 
...
%The authors discuss how to modify the encryption function in message  \mMsgtwo{} and add a ``real'' encryption function, as opposed to the current use of XOR for encryption. 
...
\mPsk{}-based method, i.e., one based on pre-shared keys for authentication.
%
...
In this method, the key schedule follows the other cases, except that the
seed for generating \mPRKtwo{} is not empty, but instead contains the
pre-shared key.
%
All three intermediate keys in the schedule are the same.
%
%The message sequence chart for the \mPsk{} method is presented in
%Figure~\ref{fig:edhocpsk}, and the following properties were verified for
