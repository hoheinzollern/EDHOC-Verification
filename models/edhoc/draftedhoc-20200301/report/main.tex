...
%
%This is one of the reasons (KARL: Göran complained earlier that this was not
...
is deleted.}
\vnote{No, we don't need to. It's enough to say it's an earlier version with fewer methods. Alessandro's paper itself mentions which version it is, with a link that still works, so that's fine.}
%
...
%
\vnote{Using the refv file to have ``cleaner'' references without extraneous details like full conference names (``Symposium on Computer Security Foundations''), conference locations, editor names for conference citations etc.}
