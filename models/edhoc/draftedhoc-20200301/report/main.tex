...
\knote{Note to self: compress this a lot}
We start with two observations on the design practices, which the \mEdhoc design
process shares with many other standards.
%
The first observation is the lack of defined security model.
%
This is quite common in standardization, and a nuisance for the formal methods
community~\cite{DBLP:conf/ccs/BasinDHRSS18}, who appear to expect industrial
standards such as IETF and 3GPP to be built ground up from a theoretical
framework.
%
Because industrial standards tend to be developed based on existing and possible
use cases, more in a top down fashion, a strict security model (or even a set of
strict security models) is difficult to define.
%
In many cases, a security model is intentionally not defined to leave more
flexibility for implementations.
%
This risks that implementations make insecure choices, but also makes it more
likely that the standard will be more widely applicable and avoids proprietary
designs.
%
Proprietary designs, in turn, also risks making insecure choices.
%
Even so, the lack of security model in \mEdhoc is somewhat troubling, in that
the standard makes a number of detailed, but imprecise, claims of properties
the protocol provides.
\knote{add example}
%
Other claims are simply incorrect, but shows the way of reasoning: assuming we
re-use most of a cryptographic core from a well analyzed protocol the security
properties shown for that protocol will carry over.
%
For example, Section 8.1. states, \emph{"EDHOC inherits its security properties
from the theoretical SIGMA-I"}, but makes no connection between the security
model in which \mSigma was analyzed and a possible security model of \mEdhoc.
%
Neither is there a mapping showing how the \mSigSig method maps to \mSigma, or
caveats that this only applies to the \mSigSig method.
%
...
security guarantees.
%
They are tailored for specific situations, and naturally cannot be entirely
equal.
%
...
come, it is easy to imagine developers downloading an \mEdhoc library, assuming
all methods are equally secure and choosing based on smallest message size,
type of credential etc, rather than on the one that provide the security
properties needed for the use-case.
%
For example, consider a use-case where \mEdhoc is run over a near-field communication
radio to derive a token from the derived session key.
%
...
group.
%
It would, however, be useful to extend the security considerations section in the
standard with example use-cases and showing the consequences of choosing
different methods.
%


NON\_REPUDIATION:---
For example, non-repudiation may be a very useful property for an access control
