...
should hold.
%
While technical limitations, such as link restrictions and that only a certain
type of credential are available to devices, are mentioned, and design
...
properties, it is hard to say whether these are the necessary or most useful
ones for IoT use cases.
%
Having a better understanding of the security properties required by these use
cases would clearly help guiding the design of \mEdhoc{}.
%
...
many not known today, so it is not possible to collect all types of use cases
that need to be considered.
%
However, this is not valid argument against collecting typical uses cases and
...
identify security properties to verify.
%
...
The simple question is whether there will be applications where device is
deployed in a hostile environment, and where it is reasonable to assume it can
be equipped with a TEE.
%
In our analysis we have chosen not to model TEEs via session state reveal
queries.
%
