...
%
Static key pairs are long-term keys used for authentication.
%
They are suitable for either regular signatures or challenge-response
signatures.
%
...
transforming points into bit strings, and we therefore abstract those as
follows.
%
Signature operations using party $A$'s key pair are denoted \mSign{A}$(\cdot)$
and \mVf{A}$(\cdot)$.
%
The DH-primitive combining a private key $d$ and a point $P$ is denoted
$\mDH(d,P)$.
%
We abuse notation and let $\mDH(\cdot)$ denote both the resulting point
and the corresponding bit string.
%
...
\mEdhoc{}'s overall goal is to establish an \mOscore{} security context,
including session key material denoted \mSessKey{}, and optionally transfer
application data \mADone{}, \mADtwo{} and \mADthree{}.
%
To accomplish this, the \mEdhoc{}
specification~\cite{our-analysis-selander-lake-edhoc-00} gives
...
%
We refer to these combinations of authentication methods simply as
...
%
Our analysis excludes multiple sessions.
%

\mEdhoc{}'s first two messages also exchange connection identifiers \mCi{} and
\mCr{}, and ephemeral public keys, \mGx{} and \mGy{}.
%
...
as well as information to authenticate that they control the corresponding
long-term keys, \mAuthi{} and \mAuthr{}.
%
...
compute $\mDH(\mX,\ \mGy)$ and $R$ compute $\mDH(\mY,\ \mGx)$, both resulting in
...
ephemeral key material, but also from $\langle\mPriv{R},\ \mPub{R}\rangle$.
%
...
for \mOscore{}, which can be considered a set that always includes \mGxy{}.
%
...
%
%There are four supported cipher suites in \mEdhoc{} -- we refer the reader to Section 3.4 of~\cite{selander-lake-edhoc-01} for the specifics of the algorithms allowed therein. Each cipher suite is identified by one of four predefined integer labels (0--3). Some algorithms are not used in some methods.  The signature algorithm and the signature algorithm curve are not used in methods without signature authentication (i.e. in \mStatStat).
%
...
%\begin{itemize}
 ...
 %       and the responder against passive attacks)
