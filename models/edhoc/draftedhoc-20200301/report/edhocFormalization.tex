...
In this section, we describe our approach and modeling choices towards formalizing the \mEdhoc{} protocol and the 
...
In addition to the basic Dolev-Yao model, we also consider two more adversary capabilities, namely long-term key reveal and ephemeral
DH half-key reveal.
%
The long-term key reveal leaks a party's long-term key to the adversary,
%
%This models compromise of the long-term key of a party $A$, 
and we denote this type of event by \mRevLTK$(A)$.
%
The event \mRevEph$(A, k)$ represents the reveal of the ephemeral DH half-key used by party $A$ while
...
events and knowledge of the agents and of the adversary.
%
For conciseness of the presentation, we use a slightly different syntax than
...
%
\footnote{In general, more than one event may have the same timestamp and hence
...
To simplify the exposition, we do not describe the properties in full detail, but they can be found in~\cite{edhocTamarinRepo}.
%
...
protocol run begun when it sends message \mMsgone{} (event type \mIStart) and the run
completed after sending message \mMsgthree{} (event type \mIComplete).
%
...
\mMsgone{} (event type \mRStart), and completed upon receiving \mMsgthree{} (event type \mRComplete). \\
%
...
and the second is $R$, where $I$ believes $R$ to be playing
the responder role. The third parameter $k$ is the established session key.
%
The parameters of the \mRComplete{} event are defined
analogously.
%
...
One way this presentation slightly differs from the \mTamarin{} code is that \mTamarin{} does not allow a disjunction on the left hand side of an implication. In the code, therefore, instead of the disjunction $\mIComplete^{t_2}(I, R, k)\, \lor\,  \mRComplete^{t_2}(I, R, k)$, we use a single action fact parametrized by $I$, $R$, and $k$ to signify that either party has completed their role. \\
%
...
%
Informally, injective agreement guarantees to an initiator $I$ that whenever $I$ completes a run ostensibly with a responder $R$,
...
authentication and secrecy, we refer to these event types as \mIStart{} and
...
\end{align*}
%
The property claims that for an initiator $I$, either the injective agreement property as described above holds, or the long-term key of the believed responder $R$ has been compromised before $I$ completed its role.
%
Note that this places no restrictions on the reveals of the ephemeral DH half-keys, or the reveal of the initiator's long-term key.
...
%their runs.
%
For the responder role we define the analogous property.
\begin{align*}
    \mPredInjR& \triangleq\
...
%------------------------------------------------------------- Implicit auth
However, unlike PFS, not all \mEdhoc{} methods enjoy the injective agreement property. So we show
...
%
If implicit agreement holds, upon completion, $A$ is guaranteed that $B$ has been or is
...
Guilhem~et~al.~\cite{DBLP:conf/csfw/GuilhemFW20} is a similar property, our definition does not
require key confirmation, so our definition is indeed closer to their implicit
authentication.
%
Also, in the code we split the property into one for the initiator $I$ and
one for $R$, but here we only show the definition for $I$ (the one for $R$ is symmetric~\footnote{The initiator and responder events swap places, and long-term key reveal events are also swapped.}).
%
...
For implicit agreement to hold, while ephemeral DH half-keys can never be revealed, the responder's long-term key
can be revealed first after the initiator completes its run.
%
Similarly, the initiator's long-term key can also be revealed, as long as it is  before any of the parties start their runs.\\
%
%\begin{figure}[h!]
%    \begin{center}
%        \tikzset{>=latex}
...
above, other properties can be indirectly inferred.
%
...
roles and session key material, all methods passing verification of those are also resistant to KCI attacks.
%
...
From the injective agreement properties it follows that each party is assured the identity of its peer upon completion.
%
...
model definitions.
%
It also provides built-in rules for the Dolev-Yao adversary and for
...
\mSpec~\cite{selander-ace-cose-ecdhe-08}, namely \mPskPsk,
...
the each party is associated with exactly one long-term key.
%
