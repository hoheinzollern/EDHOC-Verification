...
In addition to the basic Dolev-Yao model we consider two more advanced, but
yet common, adversary capabilities, namely long-term key reveal and ephemeral
DH half-key reveal.
%
...
%
The event \mRevEph$(A, k)$ represents ephemeral DH half-key reveal of party $A$
...
%
We encoded the model and properties in language of the \mTamarin{} verification
tool~\cite{DBLP:conf/cav/MeierSCB13}.
%
\mTamarin{} uses a fragment of temporal first order logic to reason about
events and knowledge of parties and the adversary.
%
For conciseness of the presentation, we use a slightly different syntax than
used by \mTamarin{}, but which has a direct mapping to \mTamarin{}'s logic.
%
...
%
We define PFS through the trace predicate
%
\begin{align*}
    \mPredPfs \triangleq
...
the responder role.
%
The first and second parameter of the \mRComplete{} event are defined
analogously.
%
...
%
Informally, injective agreement guarantees to an initiator $I$
...
for the responder.
%
For the initiator role we define injective agreement by the
trace predicate
\begin{align*}
    \mPredInjI& \triangleq
...
one for $R$, but due to the symmetry we only show the definition for $I$.
%
...
%\end{figure}
%%
The responder's guarantee \mPredImpR{} is defined analogously, letting
initiator and responder events swap places, and letting long-term key reveal
events swap places.\\
%
...
From the injective agreement properties it follows that each party upon
completion is assured about the identity of its peer.
%
...
state.
%
Rules are equipped with event annotations called actions that are conceptually
...
\mTamarin{} provides built-in rules for Dolev-Yao class adversaries and for
...
All \mTamarin{} models can be found at~\cite{edhocTamarinRepo}.
\\
...
with a public constant, as part of its input, to model different
hash functions.
%
Due to not modeling error messages, the selected hash function is fixed.
%
The HKDF interface is represented by the function symbol \mT{expa} for the
...
%
To enable adversaries to decrypt without being able to verify the integrity, we
...
We rely on \mTamarin's{} built-in message deduction rules for a Dolev-Yao class
adversary.
%
...
%
The long-term keys are revealed on registration, before protocol execution.
%
The ephemeral DH half-key of a party is revealed when the party completes, i.e.,
at events of type \mIComplete{} and \mRComplete{}.
%
A stronger, and more realistic model, would reveal the latter keys when they are
...
and the responder $R$.
%
The rules correspond to the events \mIStart{}, \mIComplete{}, \mRStart{} and
...
%
Facts prefixed with \mT{StI} carry state information between rule \mT{I1} and
rule \mT{I3}.
%
A thread unique term \mT{tid} link two rules to a given state fact.
%
Analogously, facts prefixed with \mT{StR} carry state information between the
responder role's rules.
%
Line 27-28 in the listing of \mT{R2_STAT_SIG} below shows such use of state
facts.
%
...
To implement the events \mIStart, \mIComplete, \mRStart{} and \mRComplete{}, and
to bind these to parameters, we use several \mTamarin{} actions.
%
For example, the action \mT{ExpRunningR(~tid, \$V, exp_sk, agreed)} in line 26
above implements binding an event \mRStart{} to the parameters and session key
material for the purpose of injective agreement.
%

As explained in Section~\ref{sec:agreedParams} it is not possible to show
injective agreement on session key material including \mGiy.
%
Therefore, we use different actions to emit events that include \mGiy{} in the
session key material and events that do not.
%
The session key material including \mGiy{} is referred to as \mT{imp_sk} in the
...
is equal to \mT{exp_sk} because there is no \mGiy{} to take into account.
%
...
%
As an example we now show the lemma for verifying the property \mPredPfs{}.
%
\begin{lstlisting}
lemma secrecyPFS:
   all-traces
...
\end{lstlisting}
%
Important to note is that the action \mT{CompletedRun(u, v, sk)} in line 3 is
emitted by both the rules \mT{I3} and \mT{R4}, and hence corresponds
to the disjunction of events $\mIComplete^{t_2} \lor \mRComplete^{t_2}$ in the
definition of \mPredPfs{} in Section~\ref{sec:secrecy}.
%
Similarly, the action \mT{EphKeyRev(sk)} in line 6 represents the ephemeral
DH half-key reveal is emitted for either $I$ or $R$, or both.
%
