...
\paragraph{SIG-STAT}
Next we present the mixed asymmetric mode SIG-STAT.

\begin{lstlisting}
rule I1_SIG_STAT:
  let gx = 'g'^~xx
      m1 = <'SIG', 'STAT', $cSUITE0, gx> 
  in
  [ !LTK_SIG($U, ~ltk)
  , Fr(~xx)
  , Fr(~tid)  /* Thread ID */
  ...
  [ StI1_SIG_STAT($U, ~ltk, $V, ~xx, m1, ~tid) 
  , Out(m1) ]
...
rule R2_SIG_STAT:
  ...
  [ StR2_SIG_STAT($V, ~ltk, grx, ~yy, prk_3e2m, TH_2, CIPHERTEXT_2, gx^~yy, ~tid, m1, m2)
  , Out(m2)
  ]
...
rule R4_SIG_STAT:
  let
    ...
    imp_sk =  exp_sk
  in
  [ StR2_SIG_STAT($V, ~ltk, grx, ~yy, prk_3e2m, TH_2, CIPHERTEXT_2, gxy, ~tid, m1, m2) 
  , !PK_SIG($U, pkU)
  , !PK_STAT($V, pkV)
  , In(m3)
  ]
