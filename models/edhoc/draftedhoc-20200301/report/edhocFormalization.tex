...
\paragraph{Note} We plan to show weak PCS in a future version of the model,
...
% \knote{TODO: The plan to show weak PCS, but are currently running out of memory.
%     If we get a hold of more powerful equipment before the deadline we will go
%     for it. Working on access to the security cluster now. If we won't make it
%     in time this will be removed.
% }
...
%\vnote{TODO: Could put the sort names in the typing convention used for the code.}
%
...
However the full code can be inspected at
\url{https://github.com/hoheinzollern/EDHOC-Verification/blob/2ee6e5877aa06ae90861317bfca0eee3a21c0e4f/models/edhoc/draftedhoc-20200301/edhocM4.spthy}.
...
\paragraph{\mPskPsk{}} The following rules model the symmetric key based
...
%\knote{TODO: check whether the I2 version above says what we want. It is not
%clear to me that it excludes the case that (u2 != u3 or v2 != v3) even though
% #j = #j2
%}
%\anote{that case would never appear in Tamarin, since events are atomic, if #j = #j2 then all other parameters must match. Instead the case tidR2 != tidR, u2 != u2, v3 != v2 can be satisfied when #j != #j2, though this is not the only case that violates the attack. Hope it makes sense}
\begin{lstlisting}
