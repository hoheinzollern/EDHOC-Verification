...
%
For the responder we define the property as follows.
\begin{align*}
    \mPredInjR& \triangleq\
...
$I$ and one for $R$.
%
We show only the definition for $I$ because it is symmetric to the one
for $R$.
%
\begin{align*}
    \mPredImpI& \triangleq
...
%
The full set of agreed parameters ($S_F$) is $S_P \cup \{I, \mGiy\}$
when \mGiy{} is
...
%
The XOR encryption can be seen in lines 18--21 in the listing of
...
%
In the case of \mSigSig, \mSigStat, and \mPskPsk, therefore, \mT{imp_sk} is the same as \mT{exp_sk}.%, because there is no \mGiy.%{} to take into account.
%
...
%
Similarly, \mT{EphKeyRev(sk)} in line 6 models that the ephemeral
